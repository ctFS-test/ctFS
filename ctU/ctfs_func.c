...
static inline ssize_t  ctfs_pwrite_atomic(int fd, const void *buf, size_t count, off_t offset){
	...
	if((ct_rt.fd[fd].flags & CTFS_O_ATOMIC) == 0){
		ctfs_pwrite_normal(fd, buf, count, offset);
	}
	// printf("pwrite atomic!\n");
	dax_grant_access(ct_rt.mpk[DAX_MPK_DEFAULT]);
	ino_t inode_n = ct_rt.fd[fd].inode->i_number;
	inode_rw_lock(inode_n);
	...
	if(residue != 0){
		...
		if(rem < CT_PAGE_SIZE - residue){
			char before[2][64];
			memcpy(staging + cur + residue, buf, rem);
			memcpy(staging + cur + residue + rem, base + cur + residue + rem, CT_PAGE_SIZE - residue - rem);
			swap_num = 1;
			goto end_cpy;
		}
	...
	if(rem != 0){
		...
		if(residue > ct_rt.fd[fd].inode->i_size){
